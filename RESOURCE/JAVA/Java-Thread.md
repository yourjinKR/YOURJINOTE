...
[Java](JAVA.md)에서 스레드는 **멀티 스레드**를 지원하여 하나의 프로세스 안에 한 개 이상의 스레드를 지원하는 구조이다. 이러한 멀티 스레드를 통해 **비동기식 및 병렬 애플리케이션**을 개발할 수 있다. 
...
스레드가 진행 중인 작업을 다른 스레드가 간섭하지 못하도록 [동기화](../CS/동기화.md) 과정이 중요하다.
...
> 과도한 락의 사용은 성능 저하와 [데드락](Deadlock.md)과 같은 문제가 발생한다.
...
[교착상태](Deadlock.md)란 두 개 이상의 스레드가 서로 리소스 해제를 기다리면서 모든 스레드가 영원히 멈춰버리는 현상이다.
