...
- [Java 추상 클래스 Abstract Class](Java-추상-클래스-Abstract-Class.md) 는 추상메서드를 자식 클래스가 구체화하여 그 기능을 확장하는데 목적이있다. (상속을 받아 기능을 확장시키는것) 하지만, [인터페이스](Java-인터페이스-Interface.md)는 서로 관련이 없는 클래스에서 공통적으로 사용하는 방식이 필요하지만 각각의 기능을 구현할 필요가 있는경우에 사용한다.
